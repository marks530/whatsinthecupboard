...
def say_hello(request):
    result = Order.objects.filter(customer_id=137).aggregate(count=Count('id'))
    result2 = Product.objects.aggregate(Min('unit_price'))
    result3 = OrderItem.objects.filter(product__id=333).aggregate(units_sold=Sum('quantity'))
    ...
    return render(request, 'hello.html', {'name': 'Mark', 'result': result, 'result2': result3, 'result3': result2 })
